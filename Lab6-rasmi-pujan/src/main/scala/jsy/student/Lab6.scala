...
*/
        ...
        if(next.atEnd) Failure("expected non empty atom",next)
          ...
          case ('(',next)=>re(next) match {
            case Success(r,next) =>(next.first, next.rest)match{
              ...
              case _ => Failure("didnot find )",next)
            }
            case _ => Failure("didnot find (",next)
          }
  ...
  def test(re: RegExpr, chars: List[Char])(sc: List[Char] => Boolean): Boolean = (re, chars) match {
    ...
    /***** Extra Credit Cases *****/
    case (RIntersect(re1, re2), _) => test(re1,chars){charss =>chars!=charss && test(RIntersect(re1,re2),charss)(sc)} && test(re2,chars)(sc)
    case (RNeg(re1), _) => !test(re1,chars)(charss =>chars!=charss && test(RNeg(re1),charss)(sc))
    case _=>false
  }
